...
if authentication_status:
    ...
    df_crr_stat = fn_status(df_crr, '소속부문')
    # linechart 제작을 위한 추세 dataframe (월별 & 소속부문별)
    df_crr_trnd = fn_trends(df_crr, '소속부문')
    ...
    # barchart 항목 순서 지정
    orders_crr_bar = ['개인부문', '전략부문', 'CA부문', 'MA부문', 'PA부문', '다이렉트부문'][::-1]
    # linechart 항목 순서 지정
    orders_crr_line = ['개인부문', '전략부문', 'CA부문', 'MA부문', 'PA부문', '다이렉트부문']

    # -------------------------------------------------  barchart 제작  ------------------------------------------------------
    # 차트에 필요한 리스트 제작
    lst_crr = [
        [df_crr_stat, '소속부문', '신청인원', '신청누계', 'group', 'h', crr_colors, crr_outsides, orders_crr_bar, '부문별 교육신청 현황'],
        [df_crr_stat, '소속부문', '수료인원', '수료누계', 'group', 'h', crr_colors, crr_outsides, orders_crr_bar, '부문별 교육수료 현황'],
        [df_crr_stat, '소속부문', '수료율', '', 'single', 'h', crr_colors, crr_outsides, orders_crr_bar, '부문별 수료율'],
        [df_crr_stat, '소속부문', 'IMO신청률', '', 'single', 'h', crr_colors, crr_outsides, orders_crr_bar, '부문별 IMO신청률']
        ]
    ...
    # -------------------------------------------------  linechart 제작  ------------------------------------------------------
    linelist_crr = [
        [df_crr_trnd, '소속부문', '신청누계', '소속부문별 신청인원 추이 (신청누계 기준)', '월'],
        [df_crr_trnd, '소속부문', '수료누계', '소속부문별 수료인원 추이 (수료누계 기준)', '월'],
        [df_crr_trnd, '소속부문', '수료율', '소속부문별 수료율 추이', '월'],
        [df_crr_trnd, '소속부문', 'IMO신청률', '소속부문별 IMO신청률 추이', '월'],
    ]
    # 꺾은선그래피 (부문별 신청누계)
